<?php
	 ...
	 */
	function authenticate_user($pdo, $username, $password) { // Authenticate user
		$sql = "SELECT * FROM users WHERE username = :username AND password = :password"; // SQL query to retrieve user
		$statement = pdo($pdo, $sql, ['username' => $username, 'password' => $password]); // Execute SQL query with parameters
		return $statement->fetch(); // Return the user info
	}
?>